...
class MapsForm extends StatefulWidget {
 ...
 final bool? edit; 
 const MapsForm({ Key? key, this.hospitalID, this.hospitalLang, this.hospitalLong, this.hospitalAddress, this.hospitalName, this.edit}) : super(key: key);
...
class MapsFormState extends State<MapsForm> {
  ...
  void DeleteInformation() async {
    ...
    final res = await http.post(
      Uri.parse('http://localhost:8000/api/DeleteLocation'),
      headers: {
        'Content-Type' : 'application/json; charset=UTF-8',
        'X-CSRF-TOKEN' : token,
      },
      body: jsonEncode({
        'LocationID' : widget.hospitalID
      })
    );
  ...
  Future<String> getDisplayName(LatLng point) async {
    final String url = 'https://nominatim.openstreetmap.org/reverse?format=json&lat=${point.latitude}&lon=${point.longitude}&zoom=18&addressdetails=1';
  ...
  void StoreInformation() async {
    ...
    final res = await http.post(
      Uri.parse('http://localhost:8000/api/RegisterLocation'),
      headers: {
        'Content-Type' : 'application/json; charset=UTF-8',
        'X-CSRF-TOKEN' : token,
      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
        title: Text('Insert Hospital Location'),
      ),
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: SizedBox(
                            ...
                            child: widget.edit! ? ElevatedButton(
